...
typedef map<const SVFBasicBlock*, vector<const SVFInstruction*> > block2Inst_type;
typedef map<const SVFBasicBlock*, vector<const SVFArgument*> > block2Arg_type;
typedef map<const SVFInstruction*, SVFGNode*> inst2VFGNode_type;
typedef map<const SVFArgument*, SVFGNode*> arg2VFGNode_type;
typedef map<const SVFFunction*, vector<const SVFBasicBlock*> > func2Block_type;


void tranverse_func(SVFModule* svfModule, block2Arg_type& block2Arg, func2Block_type& func2Block){

}

void tranverse_bb(SVFG* svfg, SVFModule* svfModule, block2Inst_type& block2Inst){
    json j;
    for (auto func: svfModule->getFunctionSet()){
        ...
        for (auto bb: func->getBasicBlockList()){
            ...
            block2Inst[bb] = vector<const SVFInstruction*>();
            for(auto inst: bb->getInstructionList()){
                block2Inst[bb].push_back(inst);
            }
        ...
        node_dict_type dict;
        j[func->getName()] = {{"node_list", node_list}, {"edge_list", edge_list}, {"node_attr", j1}};
    }
    auto buf = json::to_bjdata(j);
    std::ofstream outFile("bb.bin", std::ios::binary);
    outFile.write(reinterpret_cast<char*>(buf.data()), sizeof(unsigned char) * buf.size());
    outFile.close();
}
